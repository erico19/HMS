...
if(isset($_POST['submit']))
{
	...
	{
	$sql ="UPDATE admin SET adminname='{$_POST['adminname']}',email='{$_POST['email']}',status='{$_POST['select']}' WHERE adminid='{$_SESSION['adminid']}'";
	...
	{
	$sql ="INSERT INTO admin(adminname,email,status) values('{$_POST['adminname']}','{$_POST['email']}','{$_POST['select']}')";
		if($qsql = mysqli_query($con,$sql))
		{
			echo "<div class='alert alert-success'>
...
function validateform() {
    if (document.frmadminprofile.adminname.value == "") {
        ...
        return false;
    } else if (!document.frmadminprofile.email.value.match(emailExp)) {
