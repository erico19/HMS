...
if(isset($_POST['submit']))
{
	...
	{
	$sql ="INSERT INTO department(departmentname,description,status,db) values('{$_POST['departmentname']}','{$_POST['textarea']}','{$_POST['select']}','{$_SESSION['userdb']}')";
	if($qsql = mysqli_query($con,$sql))
	{
...
if(isset($_GET['editid']))
{
	$sql="SELECT * FROM department WHERE departmentid='{$_GET['editid']}' && db='{$_SESSION['userdb']}' ";
    ...
    <table class="table table-hover">
      <tbody>
        <tr>
          ...
          <td width="66%"><input placeholder=" Enter Here " class="form-control" type="text" name="departmentname" id="departmentname" value="<?php if(isset($_GET['editid'])){echo $rsedit['departmentname'];} ?>" /></td>
        </tr>
        <tr>
          <td>Description</td>
          <td><textarea placeholder=" Enter Here " class="form-control no-resize" name="textarea" id="textarea" cols="45" rows="5"><?php if(isset($_GET['editid'])){echo $rsedit['description'] ;} ?></textarea></td>
        </tr>
        <tr>
          <td>Status</td>
