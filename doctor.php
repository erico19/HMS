...
if(isset($_POST['submit']))
{
	...
	{
			$sql ="UPDATE doctor SET doctorname='{$_POST['doctorname']}',mobileno='{$_POST['mobilenumber']}',departmentid='{$_POST['select3']}',loginid='{$_POST['loginid']}',password='{$_POST['password']}',status='{$_POST['select']}',education='{$_POST['education']}',experience='{$_POST['experience']}',consultancy_charge='{$_POST['consultancy_charge']}' WHERE doctorid='{$_GET['editid']}' && db='{$_SESSION['userdb']}'";
	...
	{
	$sql ="INSERT INTO doctor(doctorname,mobileno,departmentid,loginid,password,status,education,experience,consultancy_charge,db) values('{$_POST['doctorname']}','{$_POST['mobilenumber']}','{$_POST['select3']}','{$_POST['loginid']}','{$_POST['password']}','Active','{$_POST['education']}','{$_POST['experience']}','{$_POST['consultancy_charge']}','{$_SESSION['userdb']}')";
	if($qsql = mysqli_query($con,$sql))
	{
		echo "<div class='alert alert-success'>
...
if(isset($_GET['editid']))
{
	$sql="SELECT * FROM doctor WHERE doctorid='{$_GET['editid']}' && db='{$_SESSION['userdb']}'";
		...
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			<div class="card">
				...
				<form method="post" action="" name="frmdoct" onSubmit="return validateform()" style="padding: 10px">
					...
					<div class="form-line">
					<input class="form-control" type="text" name="doctorname" id="doctorname" value="<?php if(isset($_GET['editid'])){echo $rsedit['doctorname'];} ?>" />
				</div>
				</div>
					...
					<div class="form-line">
					<input class="form-control" type="text" name="mobilenumber" id="mobilenumber" value="<?php if(isset($_GET['editid'])){echo $rsedit['mobileno'];} ?>"/>
				</div>
				</div>
			...
			<div class="form-group"><label>Department</label> 
				<div class="form-line">
					<select  name="select3" id="select3" class="form-control show-tick">
						...
						<?php
						$sqldepartment= "SELECT * FROM department WHERE status='Active' && db='{$_SESSION['userdb']}'";
					...
					<div class="form-line">
					<input class="form-control" type="password" name="password" id="password" value="<?php if(isset($_GET['editid'])){echo $rsedit['password']; } ?>"/>
				</div>
				</div>
					...
					<div class="form-line">
					<input class="form-control" type="password" name="cnfirmpassword" id="cnfirmpassword" value="<?php if(isset($_GET['editid'])){echo $rsedit['password'];} ?>"/>
				</div>
				</div>
