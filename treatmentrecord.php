...
if(isset($_POST['submit']))
{
	...
	{
		$sql ="INSERT INTO treatment_records(slipid,treatmentid,patientid,doctorid,treatment_description,uploads,treatment_date,treatment_time,status) values('{$_POST['select2']}','{$_POST['select4']}','{$_POST['patientid']}','{$_POST['select5']}','{$_POST['textarea']}','$filename','{$_POST['treatmentdate']}','{$_POST['treatmenttime']}','Active')";
...
if(isset($_GET['editid']))
{
	$sql="SELECT * FROM treatment_records WHERE treatment_records_id='{$_GET['editid']}' ";
	$qsql = mysqli_query($con,$sql);
...
if(isset($_GET['delid']))
{
	$sql ="DELETE FROM treatment_records WHERE treatment_records_id='{$_GET['delid']}'";
	$qsql=mysqli_query($con,$sql);
	if(mysqli_affected_rows($con) == 1)
	{
...
<div class="container-fluid">
  ...
  <div class="card" style="padding: 10px">
    ...
    <table class="table table-bordered table-striped">
      <tbody>
        <tr>
          <td width="40%">Slip ID</td>
          <td width="60%">
          <input class="form-control" type="text" readonly name="select2" value="<?php echo $_GET['slipid']; ?>"  /></td>
        </tr>
        <tr>
<?php
$sqlpatient= "SELECT * FROM slip_registration LEFT OUTER JOIN patient on patient.patientid=slip_registration.patientid WHERE slip_registration.db='{$_SESSION['userdb']}' AND slipid='{$_GET['slipid']}'";
...
?>	
        ...
        <tr>
          ...
          <select name="select4" id="select4" class="form-control show-tick">
           <option value="">Select</option>
            <?php
		  	$sqltreatment= "SELECT * FROM treatment WHERE status='Active' AND db='{$_SESSION['userdb']}'";
        ...
        <tr>
          ...
          <td>
    		<?php
				$sqldoctor= "SELECT * FROM doctor INNER JOIN department ON department.departmentid=doctor.departmentid WHERE doctor.status='Active' AND doctor.doctorid='{$_SESSION['doctorid']}' AND db='{$_SESSION['userdb']}'";
        ...
        <tr>
          ...
          <select name="select5" id="select5">
    		<option value="">Select</option>
    		<?php
				$sqldoctor= "SELECT * FROM doctor INNER JOIN department ON department.departmentid=doctor.departmentid WHERE doctor.status='Active' AND doctor.db='{$_SESSION['userdb']}'";
				$qsqldoctor = mysqli_query($con,$sqldoctor);
				while($rsdoctor = mysqli_fetch_array($qsqldoctor))
				{
        ...
        <tr>
          ...
          <td><input class="form-control" name="textarea" id="textarea" cols="45" rows="5"><?php if(isset($_GET['editid'])){echo $rsedit['treatment_description'] ;} ?></textarea></td>
        </tr>
        <tr>
    ...
    <p>&nbsp;</p>
        ...
        <tbody>
          ...
          <?php
		$sql ="SELECT * FROM treatment_records WHERE slipid='{$_GET['slipid']}' ";
		$qsql = mysqli_query($con,$sql);
			...
			<td>&nbsp;<a href='treatmentfiles/{$rs['uploads']}'>Download</a></td>
			 <td>&nbsp;{$rs['treatment_date']}</td>
			  <td>&nbsp;{$rs['treatment_time']}</td>
			    <td>&nbsp;{$rs['status']}</td>
          <td>&nbsp;
		  <a href='treatmentrecord.php?editid={$rs['treatment_records_id']}&slipid={$rs['slipid']}'>Edit</a>| <a href='treatmentrecord.php?delid={$rs['treatment_records_id']}&slipid={$rs['slipid']}'>Delete</a> </td>
        </tr>";
		}
		?>
        </tbody>
