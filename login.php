...
{	
	...
	$sql = "SELECT * FROM admin WHERE email='{$_POST['email']}' AND password='$pwd' AND status='Active'";
	$qsql = mysqli_query($con,$sql);
	if(mysqli_num_rows($qsql) == 1)
	{
		$rslogin = mysqli_fetch_array($qsql);
		$_SESSION['adminid']= $rslogin['adminid'] ;
		$_SESSION['userdb']= $rslogin['db'] ;
		echo "<script>window.location='dashboard';</script>";
	}
	else
	{
...
?></div>
    ...
    <div class="input-group"> <span class="input-group-addon"> <i class="zmdi zmdi-account"></i> </span>
                ...
                <div>
                    ...
                    <div class="text-left"> 
                        <a class="btn btn-sm btn-info btn-raised waves-effect float-left " href="#">Demo </a>
                        <a class="btn btn-sm btn-info btn-raised waves-effect float-right" href="createaccount">Create Account</a>
                    </div>
                </div>
            </form>
        </div>
...
function validateform()
{
	...
	else if(!document.frmadminlogin.email.value.match(emailExp))
	{
		document.getElementById("err").innerHTML ="<div class='alert alert-info'><strong>Heads up!</strong> Invalid email</div>";
		document.frmadminlogin.email.focus();
		return false;
	}
