...
?>
    ...
    <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
      ...
      <thead>
        <tr>
          <th width="15%" height="30"><div align="center">Name</div></th>
          <th width="20%"><div align="center">DOB</div></th>
          <th width="28%"><div align="center">Contact</div></th>    
          <th width="20%"><div align="center">Relation</div></th>
          
        </tr>
      </thead>
	  <tbody>
	  <?php
		$sqlfamily = "SELECT * FROM patient WHERE familyid='{$rspatient['familyid']}' ";
		$qsqlfamily = mysqli_query($con,$sqlfamily);
		while($rsfamily = mysqli_fetch_array($qsqlfamily))
		{
			echo "<tr>
			<td>{$rsfamily['patientname']}<td>
			<td>{$rsfamily['dob']} </td>
			<td>{$rsfamily['mobileno']} </td>
			<td>{$rsfamily['relation']} </td>
			</tr>";
		} ?>
      </tbody>
    </table>
