...
if(isset($_POST['submit']))
{
	...
	{
		$sql ="INSERT INTO medicine(medicinename,medicinecost,description,status,db) values('{$_POST['medicinename']}','{$_POST['medicinecost']}','{$_POST['description']}','{$_POST['status']}','{$_SESSION['userdb']}')";
		if($qsql = mysqli_query($con,$sql))
		{
...
if(isset($_GET['editid']))
{
	$sql="SELECT * FROM medicine WHERE medicineid='{$_GET['editid']}' AND db='{$_SESSION['userdb']}' ";
    ...
    <table class="table table-hover">
      <tbody>
        <tr>
          ...
          <td width="66%"><input placeholder="Enter Here" class="form-control" type="text" name="medicinename" id="medicinename" value="<?php if(isset($_GET['editid'])){echo $rsedit['medicinename'];} ?>" /></td>
        </tr>
        <tr>
          <td width="34%">Medicine cost</td>
          <td width="66%"><input placeholder="Enter Here" class="form-control" type="text" name="medicinecost" id="medicinecost" value="<?php if(isset($_GET['editid'])){echo $rsedit['medicinecost'];} ?>" /></td>
        </tr>
        <tr>
          <td>Description</td>
          <td><textarea placeholder="Enter Here" class="form-control no-resize" name="description" id="description" cols="45" rows="5"><?php if(isset($_GET['editid'])){echo $rsedit['description'] ;} ?></textarea></td>
        </tr>
        <tr>
          <td>Status</td>
