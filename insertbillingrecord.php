...
$sql ="SELECT * FROM billing WHERE slipid='{$_GET['slipid']}'";
...
if($billtype == "Room Rent")
{
	...
	//Room tariff
	$sql ="INSERT INTO billing_records( billingid, bill_type_id, bill_type, bill_amount, bill_date, status,db) VALUES ('$billid','$roomid','Room Rent','{$rsroomtariff['room_tariff']}','$dt','Active','{$_SESSION['userdb']}')";
	$qsql=mysqli_query($con,$sql);
	}
}
...
if($billtype == "Doctor Charge" && $billtype1="Treatment Cost")
{
	...
	{
		$sql ="INSERT INTO billing_records( billingid, bill_type_id, bill_type, bill_amount, bill_date, status,db) VALUES ('$billid','$doctorid','Consultancy Charge','{$rsdoctor['consultancy_charge']}','$dt','Active','{$_SESSION['userdb']}')";
		$qsql=mysqli_query($con,$sql);
	}
	...
	$sql ="INSERT INTO billing_records( billingid, bill_type_id, bill_type, bill_amount, bill_date, status,db) VALUES ('$billid','$treatmentid','Treatment','{$rstreatment['treatment_cost']}','$dt','Active','{$_SESSION['userdb']}')";
	$qsql=mysqli_query($con,$sql);
}
...
if($billtype == "Prescription charge")
{
	...
	$rstreatment = mysqli_fetch_array($qsqltreatment);
	//Prescription charge
	 $sql ="INSERT INTO billing_records(billingid, bill_type_id, bill_type, bill_amount, bill_date, status,db) VALUES ('$billid','$prescriptionid','Prescription Charge','$presamt','$dt','Active','{$_SESSION['userdb']}')";
	$qsql=mysqli_query($con,$sql);
}
...
{
//Consultancy Charge
$sql ="INSERT INTO billing_records( billingid, bill_type_id, bill_type, bill_amount, bill_date, status,db) VALUES ('$billid','$doctorid','Consultancy Charge','$billamt','$dt','Active','{$_SESSION['userdb']}')";
$qsql=mysqli_query($con,$sql);
}
...
if($billtype == "Service Charge")
{
	...
	//Prescription charge
	$sql ="INSERT INTO billing_records( billingid, bill_type_id, bill_type, bill_amount, bill_date, status,db) VALUES ('$billid','$servicetypeid','Service Charge','$servicecharge','{$_POST['date']}','Active','{$_SESSION['userdb']}')";
	$qsql=mysqli_query($con,$sql);
